...
class LegacyClient(ComponentBase):
    ...
    def __init__(self):
        ...
        self.fetcher = Fetcher(
            ...
            self.configuration.parameters[KEY_STORAGE_TABLE],
    ...
    def _parse_ssh_parameters(self):
        ...
        if ssh_config == {}:  # or ssh_config.get(KEY_SSH_USE) is False:
            ...
            try:
                ssh_object = SshTunnel(
                    ssh_config[KEY_SSH_HOST],
                    ssh_config[KEY_SSH_PORT],
                    ssh_config[KEY_SSH_USERNAME],
                    ssh_config[KEY_SSH_PKEY],
                )
    ...
    def _parse_index_parameters(self):
        ...
        if "{{date}}" in index:
            ...
            if _tz not in pytz.all_timezones:
                raise UserException(
                    f"Incorrect timezone {_tz} provided. Timezone must be a valid DB timezone name. "
            ...
            logging.info(
                f"Replaced date placeholder with value {_date_formatted}. " + f"Downloading data from index {index}."
            )
    ...
    def run(self):
        ...
        with ElasticDictWriter(
            self.fetcher.get_table_path(), fieldnames=columns, restval="", quoting=csv.QUOTE_ALL, quotechar='"'
        ) as wr:
            ...
            while not is_complete:
                ...
                if _scroll_out == "":
                    raise UserException(
                        f"Could not download data for scroll {_scroll_id}.\n" + f"STDERR: {_scroll_err}."
                    )
